...
# original_del长度为200
original_del = ['73', '583', '25', '863', '113', '137', '238', '54', '221', '607', '959', '236', '680', '300', '641',
                '976', '47', '435', '404', '186', '320', '801', '301', '414', '492', '645', '926', '370', '649', '713',
                '675', '600', '702', '135', '723', '536', '652', '240', '150', '595', '121', '574', '171', '81', '120',
                '62', '335', '955', '369', '495', '57', '970', '718', '573', '203', '996', '576', '487', '589', '478',
                '101', '754', '925', '530', '255', '165', '470', '710', '934', '921', '814', '885', '613', '569', '138',
                '817', '486', '937', '838', '93', '30', '556', '949', '664', '742', '4', '191', '575', '371', '187',
                '975', '677', '689', '197', '731', '878', '805', '548', '565', '897', '115', '832', '348', '939', '787',
                '581', '258', '393', '982', '668', '110', '225', '822', '15', '274', '729', '951', '27', '326', '907',
                '157', '415', '850', '502', '571', '531', '244', '296', '841', '317', '517', '144', '147', '950', '948',
                '881', '489', '24', '984', '410', '964', '590', '870', '95', '846', '229', '49', '630', '500', '430',
                '852', '336', '943', '685', '220', '188', '199', '482', '398', '128', '405', '550', '732', '671', '509',
                '481', '890', '14', '271', '78', '849', '736', '599', '323', '130', '374', '483', '883', '840', '477',
                '496', '862', '281', '422', '145', '479', '215', '633', '646', '297', '315', '957', '38', '947', '328',
                '227', '578', '622', '577', '380']
...
with open(csv_path, 'w') as f:
    ...
    csv_head = ["d_max", "matched_ids", "num_del", "add_time", "del_time", "search_time"]
...
for d_level in d_level_arr:
    for file_data in file_data_arr:
        ...
        for num_del in cur_del_no:
            ...
            print(">>>>>> encryption")
            start = timer()
            for i in universal_dict[keyword]:
                client.enc(keyword, str(i))
            end = timer()
            add_time = format((end - start) * 1000, '.4f')
            ...
            del_time = format((end - start) * 1000, '.4f')
            print(">>>>>> time taken " + str((end - start) * 1000) + " ms")
            ...
            server = SSE_Server(d_level, ski, iv, encrypted_index, deletion_paths, deletion_enckey_tags)
            ...
            while count < counter:
                start = timer()
                result = server.search(token, keyword_stage[keyword][3], keyword_stage[keyword][1])
                # print(len(result))
                end = timer()
            ...
            search_time = format(avg, '.4f')
            print(">>>>>> " + "average search time=" + str(avg) + " after testing " + str(counter) + " times")

            with open(csv_path, 'a+') as f:
                csv_write = csv.writer(f)
                data_row = [d_level, file_data, num_del, add_time, del_time, search_time]
